...
namespace routingrules_loadbalances
{
    ...
    {
        string[,] loadBalArr = new string[5, 4];
                    ...
                    foreach (ListViewItem item in listView2.Items)
                    {
                        ...
                        comLdBal_Cursor.CommandText = "with LBTEmp(mTid, UploadID) as " +
                                                       "(select tid, uploadhost_id from [dbo].[tmp_LB]) " +
                                                        " insert into loadbalance_test(BALANCEGROUP, DESTPORT, LOADTYPE, LOADVALUE) " +
                                                        " select mTid, '" + ldBl0_0 + "' , '" + ldBl0_2 + "','" + ldBl0_1 + "'" +
                                                       " from LBTemp " +
                                                       " where UploadID = ? ";
